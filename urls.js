...
const throughput = 5;
const urls = Array(numberOfUrls)
  .fill("url_")
  .map((url, index) => url + (index % 10));
const urlsData = Object.fromEntries(urls.map((url) => [url, url]));
...
const showSolutions = () => {
  ...
  const solution = async (urls, throughput) => {
    urls = [...urls].reverse();
  ...
  solution(urls, throughput).then((data) => console.log(data));
};
