...
const showSolutions = () => {
  ...
  const fetchEmulation = (returnedValue) => () =>
    new Promise((resolve) =>
  ...
  const solution = async (urls, workers) => {
    const fetches = urls.map((value, index) =>
      fetchEmulation(value + (index + 1))
    );
